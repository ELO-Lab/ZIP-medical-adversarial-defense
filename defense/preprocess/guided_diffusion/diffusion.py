...
class DifusionImageNet64:
    def __init__(self, device='cuda'):
        self.device = device
        from diffusers import UNet2DModel

        repo_id = "openai/diffusers-ct_imagenet64"
        # model = UNet2DModel.from_pretrained(repo_id)
        self.model = UNet2DModel.from_pretrained(repo_id, subfolder="unet")
    ...
    def __call__(self, xt, t):
        ...
        with torch.no_grad():
            class_labels = torch.randint(0, self.model.config.num_class_embeds, size=(xt.shape[0],))
            class_labels = class_labels.to(self.device)
            noisy_residual = self.model(sample=xt, timestep=t, class_labels=class_labels, return_dict=False)[0]
...
class Diffusion(object):
    ...
    def _sample(self, simplified):
        ...
        if self.config.model.type == 'simple':
            ...
            if self.config.data.dataset == "CIFAR10":
                ...
                name = 'celeba_hq'
            elif self.args.dataset == 'Alzheimer':
                print('Loading Alzheimer Diffusion')
                ckpt = '/home/chaunm/Projects/ScoreOpt/edm/training-runs3/00017-Alzheimer2-64x64-uncond-ddpmpp-edm-gpus1-batch32-fp32/network-snapshot-005001.pkl'
                # ckpt = '/home/chaunm/Projects/ScoreOpt/edm/training-runs3/00017-Alzheimer2-64x64-uncond-ddpmpp-edm-gpus1-batch32-fp32/network-snapshot-007502.pkl'
                with dnnlib.util.open_url(ckpt) as f:
                    model = pickle.load(f)['ema'].to(self.device)
            elif self.args.dataset == 'ChestCancer':
                print('Loading ChestCancer Diffusion')
                ckpt = '/home/chaunm/Projects/ScoreOpt/edm/training-runs3/00013-Chest_Cancer-64x64-uncond-ddpmpp-edm-gpus1-batch32-fp32/network-snapshot-005001.pkl'
                with dnnlib.util.open_url(ckpt) as f:
                    model = pickle.load(f)['ema'].to(self.device)
            elif self.args.dataset == 'KidneyCancer':
                print('Loading Kidney Cancer Diffusion')
                ckpt = '/home/chaunm/Projects/ScoreOpt/edm/training-runs3/00014-Kidney_Cancer-64x64-uncond-ddpmpp-edm-gpus1-batch32-fp32/network-snapshot-005001.pkl'
                with dnnlib.util.open_url(ckpt) as f:
                    model = pickle.load(f)['ema'].to(self.device)
            elif self.args.dataset == 'Monkeypox':
                print('Loading Monkeypox Diffusion')
                ckpt = '/home/chaunm/Projects/ScoreOpt/edm/training-runs3/00016-Monkeypox-64x64-uncond-ddpmpp-edm-gpus1-batch16-fp32/network-snapshot-005000.pkl'
                with dnnlib.util.open_url(ckpt) as f:
                    model = pickle.load(f)['ema'].to(self.device)
            else:
                raise ValueError
    ...
    def sample(self, simplified, test_dataset):
        ...
        if self.args.dataset == 'CelebA':
            ...
            print('Loading Alzheimer Diffusion')
            ckpt = '/home/chaunm/Projects/ScoreOpt/edm/training-runs3/00000-Alzheimer-64x64-uncond-ddpmpp-edm-gpus1-batch32-fp32/network-snapshot-007502.pkl'
            # ckpt = '/home/chaunm/Projects/ScoreOpt/edm/training-runs3/00017-Alzheimer2-64x64-uncond-ddpmpp-edm-gpus1-batch32-fp32/network-snapshot-007502.pkl'
            with dnnlib.util.open_url(ckpt) as f:
                model = pickle.load(f)['ema'].to(self.device)
        elif self.args.dataset == 'ChestCancer':
            print('Loading Chest Cancer Diffusion')
            ckpt = '/home/chaunm/Projects/ScoreOpt/edm/training-runs3/00013-Chest_Cancer-64x64-uncond-ddpmpp-edm-gpus1-batch32-fp32/network-snapshot-007502.pkl'
            with dnnlib.util.open_url(ckpt) as f:
                model = pickle.load(f)['ema'].to(self.device)
        elif self.args.dataset == 'KidneyCancer':
            print('Loading Kidney Cancer Diffusion')
        ...
        # print('load Imagenet with size', self.config.data.image_size, self.device)
        # model = DifusionImageNet64()
    ...
    def simplified_purify(self, model, test_dataset):
        ...
        IMG_SIZE = 64
        A2 = torch.nn.AdaptiveAvgPool2d((IMG_SIZE//scale,IMG_SIZE//scale))
        ...
        for x_orig, n_class in pbar:
            ...
            tvu.save_image(x_orig, f"/home/chaunm/Projects/ZIP/steps/orig_O.png")
            ...
            # print(x_orig.shape, y1.shape, y2.shape, x.shape)
            ...
            with torch.no_grad():
                ...
                # num_steps = config.time_travel.T_sampling
                # sigma_min = 0.002
                # sigma_max = 80
                # rho = 7
                # step_indices = torch.arange(num_steps, dtype=torch.float64)
                # t_steps = (sigma_max ** (1 / rho) + step_indices / (num_steps - 1) * (sigma_min ** (1 / rho) - sigma_max ** (1 / rho))) ** rho
                # t_steps = torch.cat([model.round_sigma(t_steps).detach().cpu(), torch.zeros_like(t_steps[:1])]).tolist() # t_N = 0
                # print('CCCC', t_steps)
                # time_pairs = list(zip(t_steps[:-1], t_steps[1:]))

                # print('=====', time_pairs)
                # reverse diffusion sampling
                tvu.save_image(Apy1, f"/home/chaunm/Projects/ZIP/steps/Apy1.png")
                tvu.save_image(Apy2, f"/home/chaunm/Projects/ZIP/steps/Apy2.png")
                tvu.save_image(x_orig, f"/home/chaunm/Projects/ZIP/steps/orig.png")
                for i, j in tqdm.tqdm(time_pairs):
                    ...
                    t = torch.maximum(t, torch.tensor(1e-11, dtype=t.dtype, device=t.device))
                    ...
                    if et.size(1) == 6:
                        et = et[:, :3]
                    # LINE 5, 6 IN PSUEDOCODE; N = 2
                    xt_hat1 = at.sqrt()*Apy1  + xt - Ap1(A1(xt)) + (1 - at).sqrt() * Ap1(A1(et))
                    xt_hat2 = at.sqrt()*Apy2  + xt - Ap2(A2(xt)) + (1 - at).sqrt() * Ap2(A2(et))
                    # xt_hat1 = at.sqrt()*Apy1  + xt - Ap1(A1(xt)) + (1 - at).sqrt() * Ap1(A1(et))
                    # xt_hat2 = at.sqrt()*Apy2  + xt - Ap2(A2(xt)) + (1 - at).sqrt() * Ap2(A2(et))
                    xt_hat  = (0.5 * xt_hat1 + xt_hat2)/2 # LINE 8 IN PSUEDOCODE
                    # xt_hat  = (xt_hat1 + xt_hat2)/2 # LINE 8 IN PSUEDOCODE
                    ...
                    tvu.save_image(xt_next, f"/home/chaunm/Projects/ZIP/steps/{i}.png")
                    xs.append(xt_next.to('cpu'))
                x = xs[-1]
                
            tvu.save_image(x, f"/home/chaunm/Projects/ZIP/steps/pre_final.png")
            x = [inverse_data_transform(config, xi) for xi in x]
            tvu.save_image(x, f"/home/chaunm/Projects/ZIP/steps/final.png")
            # exit()
            ...
            if self.type == "train":
                ...
                for i in range(len(x)):
                    ...
                    # tvu.save_image(
                    #     x_orig, os.path.join(self.args.test_image_folder, f"{clean_class_name}", f"{clean_class_name}_orig.png")
                    # )
                    #pois_class_name = test_dataset.classes[n_class['label_pois'][i].item()]
                    #os.makedirs(os.path.join(self.args.test_image_folder, f"{clean_class_name}", f"{pois_class_name}"), exist_ok=True)
                    # print(os.path.join(self.args.test_image_folder, f"{clean_class_name}", f"{clean_class_name}_{idx_so_far + i}.png"))
                    tvu.save_image(
